 ...
 * document-shaped window the user should align the ID card / passport with.
...
class DocumentOverlayView @JvmOverloads constructor(
    ...
    enum class Mode(val aspectRatio: Float) {
        /** ISO/IEC 7810 ID-1: 85.60 x 53.98 mm (driver's licenses, national ID cards) */
        ID_CARD(85.60f / 53.98f),

        /** Passport data page, roughly 125 x 88 mm */
        PASSPORT(125f / 88f),
    }

    var mode: Mode = Mode.ID_CARD
        set(value) {
            field = value
            recomputeFrame()
            invalidate()
    ...
    /** When true the brackets turn amber to signal glare on the document. */
    var glare: Boolean = false
        set(value) {
            if (field == value) return
            field = value
            bracketPaint.color = if (value) GLARE_COLOR else OK_COLOR
            invalidate()
        }
    ...
    private companion object {
        val OK_COLOR = Color.parseColor("#4CAF50")
        val GLARE_COLOR = Color.parseColor("#FFC107")
    }
