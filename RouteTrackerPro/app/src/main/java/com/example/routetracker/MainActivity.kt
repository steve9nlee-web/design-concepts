...
class MainActivity : AppCompatActivity() {
    ...
    private val createExcelLauncher = registerForActivityResult(
        ...
        if (uri != null) {
            ...
            Prefs.setExcelUri(this, uri.toString())
            updateExcelStatus()
            syncExcel()
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnChooseExcel.setOnClickListener {
            toast(getString(R.string.excel_pick_hint))
            createExcelLauncher.launch("RouteTracker TripLog.xlsx")
        }
        binding.btnSyncNow.setOnClickListener {
            if (Prefs.getExcelUri(this) == null) {
                toast(getString(R.string.excel_unlinked))
            } else {
                syncExcel()
            }
        }
    ...
    private fun updateExcelStatus() {
        val uri = Prefs.getExcelUri(this)
        binding.txtExcelStatus.text = if (uri == null) {
            getString(R.string.excel_unlinked)
        } else {
            ...
            }.getOrNull() ?: "TripLog.xlsx"
            getString(R.string.excel_linked, name)
        }
    }

    private fun syncExcel() {
        Thread {
            val ok = ExcelSync.syncIfConfigured(this)
            runOnUiThread {
                toast(getString(if (ok) R.string.excel_sync_ok else R.string.excel_sync_fail))
            }
        }.start()
    }
